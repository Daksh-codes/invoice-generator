...
const migrations = [
  {
    ...
    up: (db) => {
      ...
      db.prepare(
        `UPDATE invoice SET status = 'active' WHERE status IS NULL`,
      ).run();
    },
  },
  {
    ...
    up: (db) => {
      ...
      const issuers = db
        .prepare("SELECT id, invoice_prefix, quotation_prefix FROM issuer")
        .all();
  ...
  },
  {
    version: 3,
    description:
      "Add converted_from_id to invoice if missing (fix for DBs that skipped it)",
    up: (db) => {
  ...
  },
  {
    ...
    up: (db) => {
      db.exec(
        "ALTER TABLE invoice ADD COLUMN template TEXT DEFAULT 'with_logo'",
      );
    },
  },
  {
  ...
  {
    version: 6,
    description: " ADD COLUMN spacer_rows ",
    up: (db) => {
      db.exec(`ALTER TABLE invoice ADD COLUMN spacer_rows INTEGER DEFAULT 3`);
    },
  },
  {
    version: 7,
    description: " Allowing user add multiple custom labels for payment mode ",
    up: (db) => {
      db.exec(`
