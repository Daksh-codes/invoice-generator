...
import { getAllBills, getAllIssuers, updatePaymentStatus, voidBill, unvoidBill } from "../api";
...
const STATUS_STYLES = {
  unpaid:  { bg: "bg-red-50",     text: "text-red-700",    dot: "bg-red-400",    label: "Unpaid"  },
  partial: { bg: "bg-amber-50",   text: "text-amber-700",  dot: "bg-amber-400",  label: "Partial" },
  paid:    { bg: "bg-emerald-50", text: "text-emerald-700",dot: "bg-emerald-500",label: "Paid"    },
  void:    { bg: "bg-slate-100",  text: "text-slate-400",  dot: "bg-slate-300",  label: "Void"    },
};
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-xl p-6 w-full max-w-sm z-10">
        <h3 className="text-base font-semibold text-slate-800 mb-1">Void this bill?</h3>
        <p className="text-sm text-slate-500 mb-1">
          <span className="font-mono font-medium text-slate-700">{bill.bill_number}</span> will be marked as void.
        </p>
...
function PartialModal({ bill, onConfirm, onCancel }) {
  const [paidAmount, setPaidAmount] = useState(bill.paid_amount ? String(bill.paid_amount) : "");
  const [paymentMode, setPaymentMode] = useState(bill.payment_mode ?? "cash");
  const [paidDate, setPaidDate] = useState(bill.paid_date ?? new Date().toISOString().slice(0, 10));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-xl p-6 w-full max-w-sm z-10 space-y-4">
        <div>
          <h3 className="text-base font-semibold text-slate-800">Partial Payment</h3>
          <p className="text-xs text-slate-400 mt-0.5 font-mono">{bill.bill_number} · Total {formatAmount(bill.total)}</p>
        </div>

        <div className="space-y-3">
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-1">Amount Paid</label>
            <input
              autoFocus
            ...
            <div className="flex rounded-lg border border-slate-200 overflow-hidden text-xs">
              {PAYMENT_MODES.map(m => (
                <button key={m.value} type="button" onClick={() => setPaymentMode(m.value)}
                  className={`flex-1 px-2 py-1.5 transition-colors ${paymentMode === m.value ? "bg-slate-800 text-white" : "text-slate-500 hover:bg-slate-50"}`}>
                  {m.label}
                </button>
              ))}
            </div>
          ...
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-1">Payment Date</label>
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <button
            onClick={() => onConfirm({ payment_status: "partial", paid_amount: paid, payment_mode: paymentMode, paid_date: paidDate })}
...
// ── Paid Modal ─────────────────────────────────────────────────────────────
function PaidModal({ bill, onConfirm, onCancel }) {
  const [paymentMode, setPaymentMode] = useState(bill.payment_mode ?? "cash");
  const [paidDate, setPaidDate] = useState(bill.paid_date ?? new Date().toISOString().slice(0, 10));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-xl p-6 w-full max-w-sm z-10 space-y-4">
        <div>
          <h3 className="text-base font-semibold text-slate-800">Mark as Paid</h3>
          <p className="text-xs text-slate-400 mt-0.5 font-mono">{bill.bill_number} · {formatAmount(bill.total)}</p>
        </div>
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <button
            onClick={() => onConfirm({ payment_status: "paid", paid_amount: Number(bill.total), payment_mode: paymentMode, paid_date: paidDate })}
...
// ── Status Dropdown ────────────────────────────────────────────────────────
function StatusDropdown({ bill, onUpdated, onVoidRequest, onUnvoidRequest, onPartialRequest, onPaidRequest }) {
  ...
  function handleSelect(e, status) {
    e.stopPropagation();
    setOpen(false);
    if (status === "void") { onVoidRequest(bill); return; }
    if (status === "unvoid") { onUnvoidRequest(bill); return; }
    if (status === "partial") { onPartialRequest(bill); return; }
    if (status === "paid") { onPaidRequest(bill); return; }
    handleUpdateStatus({ payment_status: "unpaid", paid_amount: 0, payment_mode: null, paid_date: null });
  }
        ...
        {open && (
          <>
            ...
            <div className="absolute z-20 left-0 mt-1 bg-white rounded-lg shadow-lg border border-slate-200 py-1 min-w-[130px]">
              <button onClick={e => handleSelect(e, "unvoid")}
        ...
        <>
          ...
          <div className="absolute z-20 left-0 mt-1 bg-white rounded-lg shadow-lg border border-slate-200 py-1 min-w-[130px]">
            ...
            <div className="border-t border-slate-100 mt-1 pt-1">
              <button onClick={e => handleSelect(e, "void")}
...
// ── Payment Mode Cell ──────────────────────────────────────────────────────
function PaymentModeCell({ bill, onUpdated }) {
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const current = bill.payment_mode;

  if (bill.status === "void" || bill.payment_status === "unpaid") {
    return <span className="text-slate-300 text-xs">—</span>;
  }

  async function handleSelect(e, mode) {
    e.stopPropagation();
    setOpen(false);
  ...
  const label = PAYMENT_MODES.find(m => m.value === current)?.label ?? "Set mode";
    ...
    <div className="relative" onClick={e => e.stopPropagation()}>
      ...
      {open && (
        <>
          ...
          <div className="absolute z-20 left-0 mt-1 bg-white rounded-lg shadow-lg border border-slate-200 py-1 min-w-[130px]">
            {PAYMENT_MODES.map(m => (
              <button key={m.value} onClick={e => handleSelect(e, m.value)}
                className={`w-full text-left px-3 py-1.5 text-xs hover:bg-slate-50 ${m.value === current ? "font-semibold text-slate-800" : "text-slate-600"}`}>
                {m.label}
              </button>
            ))}
          </div>
...
export default function BillsDashboard() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [billsRes, issuersRes] = await Promise.all([getAllBills(), getAllIssuers()]);
  ...
  const handleUpdated = useCallback((billId, payload) => {
    setBills(prev => prev.map(b => b.id === billId ? { ...b, ...payload } : b));
  }, []);
  ...
  async function handleVoidConfirm() {
    ...
    try {
      ...
      setBills(prev => prev.map(b => b.id === voidTarget.id ? { ...b, status: "void" } : b));
    } catch (err) {
  ...
  async function handleUnvoidConfirm() {
    ...
    try {
      ...
      setBills(prev => prev.map(b => b.id === unvoidTarget.id ? { ...b, status: "active" } : b));
    } catch (err) {
  ...
  const filtered = useMemo(() => {
    return bills.filter(b => {
      ...
      if (filterStatus && b.payment_status?.toLowerCase() !== filterStatus) return false;
      if (filterTemplate && (b.template ?? "with_logo") !== filterTemplate) return false;
      ...
      if (search) {
        ...
        const firmName = issuers.find(i => i.id === b.issuer_id)?.firm_name ?? "";
        if (
  ...
  }, [bills, issuers, filterFirm, filterStatus, filterTemplate, filterDateFrom, filterDateTo, search]);

  const stats = useMemo(() => {
    const nonVoid = filtered.filter(b => b.status !== "void");
    const total = nonVoid.reduce((s, b) => s + Number(b.total ?? 0), 0);
    const paid = nonVoid.filter(b => b.payment_status === "paid").reduce((s, b) => s + Number(b.total ?? 0), 0);
  ...
  function clearFilters() {
    setFilterFirm(""); setFilterStatus(""); setFilterTemplate("");
    setFilterDateFrom(""); setFilterDateTo(""); setSearch("");
  }

  const hasFilters = filterFirm || filterStatus || filterTemplate || filterDateFrom || filterDateTo || search;
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <SummaryCard label="Total Bills" value={stats.count} sub={hasFilters ? "filtered" : "all time"} />
          <SummaryCard label="Total Billed" value={formatAmount(stats.total)} sub={hasFilters ? "filtered" : "all time"} />
          <SummaryCard label="Amount Paid" value={formatAmount(stats.paid)} accent="text-emerald-600" />
          <SummaryCard label="Amount Pending" value={formatAmount(stats.pending)} accent="text-amber-600" />
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3 items-end">
            ...
            <div className="col-span-2">
              <label className="text-xs font-medium text-slate-400 uppercase tracking-wider block mb-1">Search</label>
              <div className="relative">
                ...
                <input type="text" placeholder="Bill no., client, firm…" value={search}
            ...
            <div>
              <label className="text-xs font-medium text-slate-400 uppercase tracking-wider block mb-1">Firm</label>
              <select value={filterFirm} onChange={e => setFilterFirm(e.target.value)}
                ...
                {issuers.map(i => <option key={i.id} value={i.id}>{i.firm_name}</option>)}
              </select>
            </div>
            ...
            <div>
              <label className="text-xs font-medium text-slate-400 uppercase tracking-wider block mb-1">Template</label>
            ...
            <div>
              <label className="text-xs font-medium text-slate-400 uppercase tracking-wider block mb-1">From</label>
              <input type="date" value={filterDateFrom} onChange={e => setFilterDateFrom(e.target.value)}
                className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-300 bg-slate-50" />
            </div>

            <div>
              <label className="text-xs font-medium text-slate-400 uppercase tracking-wider block mb-1">To</label>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
          {filtered.length === 0 ? (
            <div className="py-16 text-center text-slate-400 text-sm">
              {hasFilters ? "No bills match your filters." : "No bills yet. Create your first bill."}
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Bill No.</th>
                    ...
                    <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Bill Date</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Paid Date</th>
                    <th className="text-right px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Amount</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Mode</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {filtered.map(bill => {
                    const firmName = issuers.find(iss => iss.id === bill.issuer_id)?.firm_name ?? "—";
                    ...
                    return (
                      <tr key={bill.id}
                        ...
                        <td className="px-4 py-3 text-slate-700 max-w-[140px] truncate">{bill.client_name ?? "—"}</td>
                        <td className="px-4 py-3 text-slate-500 max-w-[120px] truncate">{firmName}</td>
                        <td className="px-4 py-3 text-slate-500 whitespace-nowrap">{formatDate(bill.bill_date)}</td>
                        <td className="px-4 py-3 text-slate-500 whitespace-nowrap">{formatDate(bill.paid_date)}</td>
                        <td className="px-4 py-3 text-right font-medium text-slate-800 whitespace-nowrap">
                          ...
                          {bill.payment_status === "partial" && bill.paid_amount > 0 && (
                            <div className="text-xs text-amber-500 font-normal">
                              Bal {formatAmount(Number(bill.total) - Number(bill.paid_amount))}
                            </div>
                          )}
                        </td>
                        <td className="px-4 py-3">
                          <PaymentModeCell bill={bill} onUpdated={handleUpdated} />
                        </td>
                        <td className="px-4 py-3">
                          <StatusDropdown
                            bill={bill}
      ...
      {voidTarget && (
        <VoidModal bill={voidTarget} onConfirm={handleVoidConfirm} onCancel={() => setVoidTarget(null)} />
      )}
      {unvoidTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-black/30" onClick={() => setUnvoidTarget(null)} />
          <div className="relative bg-white rounded-xl shadow-xl p-6 w-full max-w-sm z-10">
            <h3 className="text-base font-semibold text-slate-800 mb-1">Unvoid this bill?</h3>
            <p className="text-sm text-slate-500 mb-5">
              <span className="font-mono font-medium text-slate-700">{unvoidTarget.bill_number}</span> will be restored to active.
            </p>
            <div className="flex justify-end gap-2">
              <button onClick={() => setUnvoidTarget(null)}
                className="px-4 py-2 text-sm text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">
                Cancel
              </button>
              <button onClick={handleUnvoidConfirm}
                className="px-4 py-2 text-sm font-medium bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors">
                Unvoid Bill
              </button>
            </div>
      ...
      {partialTarget && (
        <PartialModal bill={partialTarget} onConfirm={handleStatusConfirm} onCancel={() => setPartialTarget(null)} />
      )}
      {paidTarget && (
        <PaidModal bill={paidTarget} onConfirm={handleStatusConfirm} onCancel={() => setPaidTarget(null)} />
      )}
    </div>
  );
}