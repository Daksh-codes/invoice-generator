      ...
      <thead>
        {plain ? (
          ...
          <tr className="bg-slate-800 text-white">
            ...
            <th className="border border-slate-700 p-2 text-right text-sm font-medium w-[35mm]">
              Amount (₹)
            </th>
          </tr>
        )}
...
// ── Shared: Pinned footer — bank table left, QR + signature right ──────────
function PinnedFooter({ data, firmName }) {
  ...
  return (
    <div className="px-10 pb-8 print:px-6 print:pb-6">
      <div className="border-t border-slate-300 pt-4 flex justify-between items-end gap-4">
        {/* Bank details */}
        {hasBank && (
          <table className="border border-collapse text-[11px] w-[90mm]">
            <tbody>
              <tr>
                <td
                  colSpan={2}
                  className="border border-slate-200 p-1 text-center font-bold bg-slate-100 text-slate-700"
                >
              ...
              {[
                ...
                value ? (
                  <tr key={label}>
                    <td className="border border-slate-200 px-2 py-0.5 font-medium text-slate-500 w-16">
                      {label}
                    </td>
                    <td className="border border-slate-200 px-2 py-0.5 text-slate-700">
                      {value}
                    </td>
                  </tr>
                ) : null,
          ...
          {hasQR && (
            <div className="flex flex-col items-center">
              <img
                src={imageUrl(upi_qr)}
                alt="UPI QR"
                className="w-[26mm] h-[26mm] object-contain border border-slate-200"
              />
...
function PaymentSummary({ data }) {
  ...
  if (!status || status === "unpaid" || paidAmount <= 0) {
    return null;
  }
  ...
  return (
    <div className="mt-4 px-10 print:px-6">
      {status === "partial" ? (
        ...
        <div className="rounded-lg bg-emerald-50 border border-emerald-200 px-4 py-3 text-emerald-900">
...
function WithLogoTemplate({ data }) {
  ...
  return (
    <div className="flex flex-col w-[210mm] min-h-[297mm] print:h-[297mm] print:overflow-hidden bg-white font-sans print:w-full">
      <div className="px-10 pt-8 print:px-6 print:pt-6">
        ...
        <div className="flex items-center gap-5 pb-4 border-b-2 border-slate-800">
          ...
          <div className="flex-1 min-w-0">
            ...
            {firm_address && (
              <p className="text-xs text-slate-400 mt-0.5">{firm_address}</p>
            )}
