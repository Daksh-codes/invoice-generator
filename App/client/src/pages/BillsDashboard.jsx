...
function ModeDropdown({ current, modes, onSelect, onAddMode }) {
  ...
  return (
    <div ref={ref} className="relative inline-block">
      <button
        onClick={() => setOpen((o) => !o)}
        className="text-xs text-slate-600 inline-flex items-center gap-2 rounded-md bg-transparent px-3 py-2 hover:bg-slate-50 transition-colors"
      >
        <span>{current ?? "Set mode"}</span>
...
function ConfirmModal({ title, message, onConfirm, onCancel, confirmLabel }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="fixed inset-0 bg-black/30" onClick={onCancel} />
      <div className="relative bg-white rounded-xl shadow-xl p-6 w-full max-w-sm z-10">
    ...
    <>
      ...
      {confirmOpen && pendingMode && (
        <ConfirmModal
          ...
          message={`Change payment mode from ${bill.payment_mode ?? "—"} to ${pendingMode}?`}
          onConfirm={handleConfirm}
          onCancel={handleCancel}
          confirmLabel={saving ? "Saving..." : "Change"}
        />
      )}
    </>
  );
                  ...
                  {filtered.map((bill) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-slate-700 max-w-35 truncate">
                          {bill.client_name ?? "—"}
                        </td>
                        <td className="px-4 py-3 text-slate-500 max-w-30 truncate">
                          {firmName}
                        </td>
                        ...
                        <td className="px-4 py-3 text-right font-medium text-slate-800 whitespace-nowrap">
                          ...
                          {bill.payment_status === "paid" && bill.paid_amount > 0 && (
